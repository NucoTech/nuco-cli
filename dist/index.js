...
var repository = "https://github.com/NucoTech/nuco-cli.git";
var author = "HerbertHe <Herbert.He0229@gmail.com>";
var license = "MIT";
var bugs = {
...
class BottomBar extends baseUI {
  ...
  write(message) {
    ...
    this.rl.setPrompt(_$1.last(msgLines));
...
var fetchAsyncQuestionProperty = function (question, prop, answers) {
  if (!_$2.isFunction(question[prop])) {
...
var require$$0 = /*@__PURE__*/getAugmentedNamespace(operators);
...
class PromptUI extends baseUI {
  ...
  run(questions, answers) {
    ...
    if (_$3.isPlainObject(answers)) {
      this.answers = _$3.clone(answers);
    } else {
      this.answers = {};
    }

    // Make sure questions is an array.
    if (_$3.isPlainObject(questions)) {
      questions = [questions];
    }
    ...
    var obs = _$3.isArray(questions) ? from$2(questions) : questions;
    ...
    return this.process
      .pipe(
        reduce$1((answers, answer) => {
          _$3.set(answers, answer.name, answer.answer);
          return answers;
        }, this.answers)
      )
      .toPromise(Promise)
  ...
  filterIfRunnable(question) {
    ...
    if (!_$3.isFunction(question.when)) {
...
var choice = class Choice {
  constructor(val, answers) {
    ...
    if (_$4.isString(val) || _$4.isNumber(val)) {
    ...
    if (_$4.isFunction(val.disabled)) {
...
var choices = class Choices {
  ...
  getChoice(selector) {
    assert__default['default'](_$5.isNumber(selector));
  ...
  where(whereClause) {
    return _$5.filter(this.realChoices, whereClause);
  }
  ...
  pluck(propertyName) {
    return _$5.map(this.realChoices, propertyName);
  }
  ...
  find(func) {
    return _$5.find(this.choices, func);
  }

  push() {
    var objs = _$5.map(arguments, (val) => new choice(val));
...
class ScreenManager {
  ...
  forceLineReturn(content, width) {
    width = width || this.normalizedCliWidth();
    return _$6.flatten(this.breakLines(content.split('\n'), width)).join('\n');
  }
}
...
class Prompt {
  constructor(question, rl, answers) {
    ...
    this.opt = _$7.defaults(_$7.clone(question), {
      validate: () => true,
      filter: (val) => val,
      when: () => true,
      suffix: '',
  ...
  handleSubmitEvents(submit) {
    ...
    var success = validation.pipe(
      filter$3((state) => state.isValid === true),
      take$1(1)
    );
...
var events = function (rl) {
  var keypress = fromEvent$1(rl.input, 'keypress', normalizeKeypressEvents)
    ...
    .pipe(filter$4(({ key }) => key.name !== 'enter' && key.name !== 'return'));
  ...
  return {
    ...
    numberKey: keypress.pipe(
      filter$4((e) => e.value && '123456789'.indexOf(e.value) >= 0),
      map$2((e) => Number(e.value)),
      share$2()
    ),

    spaceKey: keypress.pipe(
      filter$4(({ key }) => key && key.name === 'space'),
      share$2()
    ),
    aKey: keypress.pipe(
      filter$4(({ key }) => key && key.name === 'a'),
      share$2()
    ),
    iKey: keypress.pipe(
      filter$4(({ key }) => key && key.name === 'i'),
      share$2()
    ),
  };
};
...
class Paginator {
  ...
  paginate(output, active, pageSize) {
    ...
    if (this.screen) {
      ...
      active = _$8.sum(lines.map((lineParts) => lineParts.length).splice(0, active));
      lines = _$8.flatten(lines);
    }
  ...
  getInfiniteLines(lines, active, pageSize) {
    ...
    var infinite = _$8.flatten([lines, lines, lines]);
...
var incrementListIndex_1 = incrementListIndex;
...
class ListPrompt extends base {
  constructor(questions, rl, answers) {
    ...
    if (_$9.isNumber(def) && def >= 0 && def < this.opt.choices.realLength) {
      this.selected = def;
    } else if (!_$9.isNumber(def) && def != null) {
      let index = _$9.findIndex(this.opt.choices.realChoices, ({ value }) => value === def);
  ...
  _run(cb) {
    ...
    events$1.line
      .pipe(
        take$2(1),
        map$3(this.getCurrentValue.bind(this)),
        flatMap$2((value) => runAsync_1(self.opt.filter)(value).catch((err) => err))
      )
   ...
   */
  onUpKey() {
    this.selected = incrementListIndex_1(this.selected, 'up', this.opt);
    this.render();
  }

  onDownKey() {
    this.selected = incrementListIndex_1(this.selected, 'down', this.opt);
    this.render();
  }
...
function listRender(choices, pointer) {
  ...
  choices.forEach((choice, i) => {
    ...
    if (choice.disabled) {
      separatorOffset++;
      output += '  - ' + choice.name;
      output += ' (' + (_$9.isString(choice.disabled) ? choice.disabled : 'Disabled') + ')';
      output += '\n';
      return;
    }
...
class InputPrompt extends base {
  ...
  _run(cb) {
    ...
    var events$1 = events(this.rl);
    var submit = events$1.line.pipe(map$4(this.filterInput.bind(this)));
...
class ConfirmPrompt extends base {
  constructor(questions, rl, answers) {
    ...
    if (_$a.isBoolean(this.opt.default)) {
...
class RawListPrompt extends base {
  constructor(questions, rl, answers) {
    ...
    var def = this.opt.default;
    if (_$b.isNumber(def) && def >= 0 && def < this.opt.choices.realLength) {
      this.selected = def;
      this.rawDefault = def;
    } else if (!_$b.isNumber(def) && def != null) {
      let index = _$b.findIndex(this.opt.choices.realChoices, ({ value }) => value === def);
  ...
  _run(cb) {
    ...
    // Once user confirm (enter key)
    var events$1 = events(this.rl);
    var submit = events$1.line.pipe(map$5(this.getCurrentValue.bind(this)));
  ...
  onArrowKey(type) {
    this.selected = incrementListIndex_1(this.selected, type, this.opt);
...
class CheckboxPrompt extends base {
  constructor(questions, rl, answers) {
    ...
    if (_$d.isArray(this.opt.default)) {
  ...
  _run(cb) {
    ...
    var validation = this.handleSubmitEvents(
      events$1.line.pipe(map$7(this.getCurrentValue.bind(this)))
    );
    validation.success.forEach(this.onEnd.bind(this));
    validation.error.forEach(this.onError.bind(this));
  ...
  getCurrentValue() {
    ...
    this.selection = _$d.map(choices, 'short');
    return _$d.map(choices, 'value');
  }

  onUpKey() {
    this.pointer = incrementListIndex_1(this.pointer, 'up', this.opt);
    this.render();
  }

  onDownKey() {
    this.pointer = incrementListIndex_1(this.pointer, 'down', this.opt);
    this.render();
  }
...
function renderChoices$2(choices, pointer) {
  ...
  choices.forEach(function (choice, i) {
    ...
    if (choice.disabled) {
      separatorOffset++;
      output += ' - ' + choice.name;
      output += ' (' + (_$d.isString(choice.disabled) ? choice.disabled : 'Disabled') + ')';
    } else {
...
class PasswordPrompt extends base {
  ...
  _run(cb) {
    ...
    // Once user confirm (enter key)
    var submit = events$1.line.pipe(map$8(this.filterInput.bind(this)));
...
function _rmdirRecursiveSync(root) {
  ...
  do {
    ...
    for (var i = 0, length = files.length; i < length; i++) {
      var
        file = path__default['default'].join(dir, files[i]),
...
var CreateFileError = /** @class */ (function (_super) {
    __extends$1(CreateFileError, _super);
...
var CreateFileError_2 = CreateFileError;

var CreateFileError_1 = /*#__PURE__*/Object.defineProperty({
	CreateFileError: CreateFileError_2
...
var LaunchEditorError_2 = LaunchEditorError;

var LaunchEditorError_1 = /*#__PURE__*/Object.defineProperty({
	LaunchEditorError: LaunchEditorError_2
}, '__esModule', {value: true});
...
var ReadFileError = /** @class */ (function (_super) {
    __extends$3(ReadFileError, _super);
...
var ReadFileError_2 = ReadFileError;

var ReadFileError_1 = /*#__PURE__*/Object.defineProperty({
	ReadFileError: ReadFileError_2
}, '__esModule', {value: true});
...
var RemoveFileError = /** @class */ (function (_super) {
    __extends$4(RemoveFileError, _super);
...
var RemoveFileError_2 = RemoveFileError;

var RemoveFileError_1 = /*#__PURE__*/Object.defineProperty({
	RemoveFileError: RemoveFileError_2
}, '__esModule', {value: true});
...
var CreateFileError$1 = CreateFileError_1.CreateFileError;

var LaunchEditorError$1 = LaunchEditorError_1.LaunchEditorError;

var ReadFileError$1 = ReadFileError_1.ReadFileError;

var RemoveFileError$1 = RemoveFileError_1.RemoveFileError;
...
var ExternalEditor = /** @class */ (function () {
    ...
    ExternalEditor.prototype.launchEditor = function () {
        try {
            var editorProcess = childProcess__default['default'].spawnSync(this.editor.bin, this.editor.args.concat([this.tempFile]), { stdio: "inherit" });
    ...
    ExternalEditor.prototype.launchEditorAsync = function (callback) {
        var _this = this;
        try {
            var editorProcess = childProcess__default['default'].spawn(this.editor.bin, this.editor.args.concat([this.tempFile]), { stdio: "inherit" });
...
var ExternalEditor_1 = ExternalEditor;

var main$2 = /*#__PURE__*/Object.defineProperty({
	CreateFileError: CreateFileError$1,
	LaunchEditorError: LaunchEditorError$1,
	ReadFileError: ReadFileError$1,
	RemoveFileError: RemoveFileError$1,
	edit: edit_1,
	editAsync: editAsync_1,
	ExternalEditor: ExternalEditor_1
}, '__esModule', {value: true});
...
var editAsync$1 = main$2.editAsync;
...
var RAW_REACT_REPO = "https://github.com/HerbertHe/raw-react";
/**
 * easy-react 仓库地址
 */

var EASY_REACT_REPO = "";
var GithubRegEx = /<a class="js-navigation-open link-gray-dark" title="(.*?)" data-pjax="#repo-content-pjax-container" href="(.*?)">/g;
...
var checkIfGitHubDir = function checkIfGitHubDir(link) {
...
var getGitHubFiles = /*#__PURE__*/function () {
  var _ref = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee(url, dir) {
    var spinner, response, body;
    return regenerator.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            spinner = ora("\u6B63\u5728\u4ECE ".concat(url, " \u83B7\u53D6\u6A21\u677F\u4FE1\u606F~")).start();
            _context.next = 3;
            return fetch(url);
          ...
          case 3:
            ...
            body.then(function (data) {
              ...
              var files = GithubRegEx.exec(data);
              ...
              if (!files) {
                ...
                if (checkIfGitHubDir(files[2])) ; else {
                  // 不是目录
                  console.log("不是目录");
                }
              }
  ...
  return function getGitHubFiles(_x, _x2) {
...
var newProject = function newProject() {
  ...
  inquirer_1.prompt(questions).then(function (answers) {
    var dirTemp = path__default['default'].resolve(__dirname); // 中断项目创建

    if (!answers.projSure) {
      console.log(source.bgRed("Tips: "), source.red("用户手动终止了项目创建"));
      return;
    } // 判断项目创建位置
    ...
    if (answers.projName.length === 0) {
      // 当前目录创建
      console.log(source.green("正在当前目录下创建新的项目~"));
    } else {
      console.log(source.green("\u6B63\u5728\u5B50\u76EE\u5F55 ".concat(answers.projName, " \u4E0B\u521B\u5EFA\u65B0\u7684\u9879\u76EE~")));
      dirTemp = path__default['default'].resolve(__dirname, answers.projName);
      console.log(dirTemp);
    } // 开始创建


    console.log(source.magentaBright("\u9879\u76EE\u6A21\u677F\u4E3A: ".concat(answers.projTemp))); // 选择目标

    switch (answers.projTemp) {
      case "raw-react":
        {
          getGitHubFiles(RAW_REACT_REPO, dirTemp)["catch"](function (err) {
            console.log("\n", source.bgRed("Error:"), source.red(err));
          });
          break;
        }
      ...
      case "easy-react":
        {
          getGitHubFiles(EASY_REACT_REPO, dirTemp)["catch"](function (err) {
            console.log("\n", source.bgRed("Error:"), source.red(err));
          });
          break;
        }
    ...
    if (error.isTtyError) {
      ...
      console.log(source.bgRed("当前环境不能加载此命令行"));
    } else {
      // Something else went wrong
      console.log(source.bgRed("Error:", source.red(error)));
    }
  });
};

var program = new commander.Command(); // 链接版本

program.version(pkg.version); // 初始化命令

program.command("new").description("新建项目工程").action(function () {
  newProject();
}); // API文档生成命令
// 需要输入文件夹和输出文件夹

program.command("api").description("生成API文档").action(function () {
  console.log("API文档生成");
}); // 文档生成命令

program.command("docs").description("生成项目文档").action(function () {
  console.log("生成项目文档");
});
program.parse(process.argv);
